...
Across a benchmark suite of 350 synthetic UI pairs spanning 5 bottleneck categories and 8 UI archetypes, \tool{} achieves 94.3\% F1 on regression detection---outperforming KLM-GOMS baselines by 31 points and static-complexity heuristics by 42 points.
Rank correlation with ground-truth cost orderings reaches Spearman $\rho = 0.91$, and bottleneck classification achieves 87\% macro-F1 across 5 cognitive failure modes.
...
\begin{enumerate}[leftmargin=*,nosep]
  ...
  \item \textbf{Comprehensive empirical evaluation} (\Cref{sec:experiments}): 5 experiments demonstrating 94.3\% F1 regression detection, Spearman $\rho = 0.91$ rank correlation, sub-60s CI/CD latency, 87\% bottleneck classification, and ablation analysis.
\end{enumerate}
...
\midrule
\tool{} (full)            & \textbf{95.1} & \textbf{96.0} & \textbf{92.8} & \textbf{94.3} \\
KLM-GOMS additive        & 72.3 & 68.5 & 70.4 & 69.4 \\
Static complexity         & 61.4 & 57.2 & 54.8 & 56.0 \\
Heuristic checklist       & 66.9 & 63.1 & 62.0 & 62.5 \\
Random baseline           & 50.3 & 50.1 & 49.8 & 50.0 \\
\bottomrule
\end{tabular}
\end{table}

\tool{} outperforms the strongest baseline (KLM-GOMS) by 24.9 F1 points.
...
\midrule
\tool{} (full)      & \textbf{0.91} & \textbf{0.78} \\
KLM-GOMS additive   & 0.64 & 0.47 \\
Static complexity    & 0.43 & 0.31 \\
Random baseline      & 0.02 & 0.01 \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
10    & 0.08  & 0.01  & \checkmark \\
50    & 0.31  & 0.04  & \checkmark \\
100   & 0.89  & 0.12  & \checkmark \\
200   & 2.45  & 0.31  & \checkmark \\
500   & 12.7  & 1.8   & \checkmark \\
1000  & 38.4  & 4.2   & \checkmark \\
2000  & 89.1  & 9.7   & partial \\
5000  & 312   & 28    & $\times$ \\
\bottomrule
\end{tabular}
\end{table}

The scaling model fits $T(n) \approx 0.0003 \cdot n^{1.72}$ ($R^2 = 0.997$), confirming sub-quadratic scaling.
For $n \leq 500$, all analyses complete within 13 seconds, well within CI/CD budgets.
...
Cross-channel interf.  & 85.0 & 82.5 & 83.7 \\
\midrule
\textbf{Macro average} & \textbf{88.5} & \textbf{86.0} & \textbf{87.2} \\
\bottomrule
\end{tabular}
\end{table}

Motor difficulty is easiest to classify (clear Fitts' law signature); memory decay is hardest (requires temporal analysis of state sequences).
...
No cost algebra (additive)     & 82.1 & $-$12.2 \\
...
The cost algebra contributes the largest marginal improvement ($+$12.2 F1), confirming that modeling cognitive interactions is the primary source of advantage over additive baselines.
Monte Carlo sampling and working memory each contribute 4--6 points.
...
The \emph{minimal} variant (Fitts+Hick only) at 69.4\% F1 is instructive: it shows that even a naive additive model of the two most basic cognitive laws provides non-trivial regression detection, but falls far short of the full system.
...
\midrule
0.5  & 82.1 & 79.3 & 83.2 & 81.2 \\
1.0  & 90.6 & 91.0 & 88.4 & 89.7 \\
2.0  & 93.1 & 94.2 & 91.0 & 92.6 \\
5.0  & \textbf{95.1} & \textbf{96.0} & \textbf{92.8} & \textbf{94.3} \\
10.0 & 94.6 & 95.8 & 91.6 & 93.7 \\
20.0 & 93.4 & 95.2 & 89.6 & 92.3 \\
\bottomrule
\end{tabular}
\end{table}
...
\tool{} is the first automated usability regression oracle that operates within CI/CD constraints, achieving 94.3\% F1 on regression detection with formal error bounds.
