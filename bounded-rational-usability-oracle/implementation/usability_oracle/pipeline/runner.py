        ...
        try:
            ...
            if mdp_a:
                policy_a = self._run_stage(
                    ...
                    mdp=mdp_a, beta=cfg.oracle.policy.beta_range[0],
                )
            ...
            if mdp_b:
                policy_b = self._run_stage(
                    PipelineStage.POLICY, cfg, result,
                    mdp=mdp_b, beta=cfg.oracle.policy.beta_range[0],
                    cache_suffix="_b",
                )
            ...
            repair_result = None
            if cfg.is_stage_enabled(PipelineStage.REPAIR):
                repair_result = self._run_stage(
                    PipelineStage.REPAIR, cfg, result,
                    mdp=mdp_a, bottlenecks=bottlenecks or [],
                )
            ...
            if cfg.is_stage_enabled(PipelineStage.OUTPUT):
                self._run_stage(
                    PipelineStage.OUTPUT, cfg, result,
                    pipeline_result=result.final_result,
