...
class FullPipelineConfig:
    ...
    def validate(self) -> list[str]:
        ...
        if self.oracle.policy.beta_min >= self.oracle.policy.beta_max:
            errors.append(
                f"beta_min ({self.oracle.policy.beta_min}) must be < "
                f"beta_max ({self.oracle.policy.beta_max})"
            )
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "oracle": {
                "parser": {"max_depth": self.oracle.parser.max_depth,
                ...
                "policy": {
                    "beta_min": self.oracle.policy.beta_min,
                    "beta_max": self.oracle.policy.beta_max,
                    "beta_steps": self.oracle.policy.beta_steps,
                },
                "repair": {
                    "max_repairs": self.oracle.repair.max_repairs,
    ...
    def from_dict(cls, d: dict[str, Any]) -> FullPipelineConfig:
        ...
        oracle.parser = ParserConfig(
            max_depth=parser_data.get("max_depth", 50),
        ...
        oracle.policy = PolicyConfig(
            beta_min=pol_data.get("beta_min", 0.1),
            beta_max=pol_data.get("beta_max", 20.0),
        ...
        oracle.repair = RepairConfig(
            max_repairs=rep_data.get("max_repairs", 10),
