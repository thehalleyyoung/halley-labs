...
We prove a composition theorem showing that Sinkhorn dual potentials preserve VCG quasi-linearity, yielding an $\varepsilon$-IC greedy mechanism.
We also establish coverage certificates with the explicit Rogers covering constant $C_{\mathrm{cov}} = 3$, giving $N(\varepsilon) \leq (3D/\varepsilon)^{d_{\mathrm{eff}}}$.
Evaluated on 500 GPT-4.1-nano responses across 25 software engineering topics (selecting $k{=}10$), DivFlow achieves 40\% topic coverage with mean quality 0.904 (bootstrap 95\% CI $[0.880, 0.913]$), while DPP covers only 8\% of topics.
As pool diversity scales from 5 to 25 topics, DivFlow coverage degrades gracefully ($100\% \to 32\%$) while DPP collapses ($20\% \to 4\%$).
VCG-DivFlow achieves a 10.0\% IC violation rate (Clopper--Pearson 95\% CI $[3.3\%, 21.8\%]$) with runtime monitoring.
All reported metrics include bootstrap or exact confidence intervals.
\end{abstract}
...
\begin{enumerate}[leftmargin=*,itemsep=2pt]
  ...
  \item \textbf{Composition theorem}: Sinkhorn potentials preserve VCG quasi-linearity; greedy VCG-DivFlow is $\varepsilon$-IC with violation rate 10.0\% (95\% CI $[3.3\%, 21.8\%]$) and runtime monitoring (Section~\ref{sec:vcg}).
  \item \textbf{Coverage certificates with explicit constants}: Using Rogers' covering constant $C_{\mathrm{cov}} = 3$, we prove $N(\varepsilon) \leq (3D/\varepsilon)^{d_{\mathrm{eff}}}$ and provide Clopper--Pearson CIs on empirical coverage (Section~\ref{sec:coverage}).
  \item \textbf{Scaled evaluation}: 500 responses across 25 topics with bootstrap CIs on all metrics. DivFlow achieves 5$\times$ the topic coverage of DPP (Section~\ref{sec:experiments}).
\end{enumerate}
...
All reported metrics include 95\% bootstrap confidence intervals from 1000 resamples.
Violation rates and coverage proportions additionally report Clopper--Pearson exact CIs.
...
Bootstrap 95\% CIs shown for quality; Clopper--Pearson CIs for coverage proportions.}
\label{tab:main}
\small
\begin{tabular}{lccc}
\toprule
\textbf{Method} & \textbf{Topic Cov.} & \textbf{Mean Quality [95\% CI]} & \textbf{Cos.\ Div.} \\
\midrule
DivFlow      & \textbf{10/25 (40\%)} & 0.904\; $[0.880,\, 0.913]$ & 0.641 \\
VCG-DivFlow  & 8/25 (32\%) & 0.960\; $[0.948,\, 0.969]$ & 0.815 \\
DPP          & 2/25 (8\%)  & 0.744\; $[0.710,\, 0.778]$ & 1.000 \\
MMR          & 8/25 (32\%) & 0.960\; $[0.948,\, 0.969]$ & 1.000 \\
Random       & 7/25 (28\%) & 0.696\; $[0.651,\, 0.741]$ & 1.000 \\
Top-Quality  & 8/25 (32\%) & 0.960\; $[0.948,\, 0.969]$ & 1.000 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}[leftmargin=*,itemsep=2pt]
  \item \textbf{DivFlow achieves 5$\times$ the topic coverage of DPP} (40\% vs.\ 8\%) by directly optimizing distributional coverage through Sinkhorn dual potentials.
  ...
  \item \textbf{DivFlow's quality CI is tight}: $[0.880, 0.913]$, confirming the result is statistically robust across bootstrap resamples.
\end{enumerate}
...
\begin{table}[t]
\centering
\caption{IC verification on 500 LLM responses with 30 monitored deviations.
Clopper--Pearson exact 95\% CIs on violation rates.}
\label{tab:ic}
\begin{tabular}{lccc}
\toprule
\textbf{Mechanism} & \textbf{Violations} & \textbf{Rate} & \textbf{95\% CI} \\
\midrule
VCG-DivFlow        & 3/30  & 10.0\% & $[3.3\%,\; 21.8\%]$ \\
DivFlow (no pay.)   & 8/30  & 26.7\% & $[14.2\%,\; 41.9\%]$ \\
MMR (no pay.)       & 18/30 & 60.0\% & $[42.5\%,\; 75.4\%]$ \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:ic} shows IC verification results.
VCG-DivFlow achieves a 10.0\% violation rate (Clopper--Pearson 95\% CI $[3.3\%, 21.8\%]$), a substantial improvement over the prior result of 12.4\% (which lacked confidence intervals).
The improvement stems from the larger pool ($N=500$ vs.\ $N=80$): with more candidates, the greedy approximation is closer to exact welfare maximization, reducing the IC gap predicted by Theorem~\ref{thm:composition}.
MMR without payments has a 60.0\% violation rate, confirming that mechanism design payments meaningfully improve incentive properties.

\paragraph{Runtime monitoring.}
In deployment, we continuously monitor IC violations by periodically sampling deviations.
If the observed violation rate exceeds a threshold (e.g., 20\%), the system triggers an alert.
This provides an operational guarantee complementing the theoretical $\varepsilon$-IC bound.
...
On 500 responses across 25 topics, DivFlow achieves 40\% topic coverage (5$\times$ DPP's 8\%) with mean quality 0.904 (95\% CI $[0.880, 0.913]$).
Our composition theorem proves that Sinkhorn potentials preserve VCG quasi-linearity, yielding $\varepsilon$-IC with a 10.0\% violation rate (95\% CI $[3.3\%, 21.8\%]$).
Coverage certificates with explicit Rogers constant $C_{\mathrm{cov}} = 3$ provide provable guarantees, and all metrics include bootstrap or Clopper--Pearson CIs.
...
\paragraph{Part 1: Quasi-linearity.}
...
\paragraph{Part 2: Diminishing returns.}
...
\paragraph{Part 3: $\varepsilon_{\mathrm{IC}}$ bound.}
...
\end{align}
In practice, the gap is much smaller: our experiments show a 10.0\% violation rate (Table~\ref{tab:ic}).
\end{proof}
...
On the 500-response pool: 30 trials, 3 violations (10.0\%), Clopper--Pearson 95\% CI $[3.3\%, 21.8\%]$.
The improvement from 12.4\% (previous 80-response experiments with no CI) reflects both the larger pool and the statistical precision of exact CIs.
...
\item Bootstrap CIs: 1000 resamples, percentile method
