...
verifiable proof certificates, a formal denotational semantics, and
...
\litmus{} checks 137 litmus-test patterns---including classical idioms,
...
variants, multi-copy atomicity tests, and dependency chains---across
10 architecture configurations (4~CPU, 6~GPU scope instantiations),
producing 1{,}370 test-model pairs.
\emph{Every} pair carries an extractable Z3 proof certificate
(1{,}095~UNSAT safety proofs, 275~SAT unsafety witnesses, zero timeouts).
All SMT-LIB2 encodings are exported as independently verifiable
\texttt{.smt2} files via the \texttt{--emit-certificate} CLI flag.
Beyond bare SAT/UNSAT verdicts, all 1{,}095 UNSAT verdicts now
carry Alethe-format resolution proof certificates, and all 275 SAT
verdicts carry self-certifying counterexample models, achieving a
100\% proof extraction rate with an average of 109.3 proof steps
per certificate---upgrading every verdict to an independently
...
independently sourced checks (33 published litmus test outcomes,
32 PTX specification guarantees, 23 Vulkan specification guarantees,
and 6 known GPU concurrency bugs), all passing.
Machine-checked fence proofs cover all unsafe CPU pairs
(99~UNSAT fence-sufficient + 72~SAT inherently observable).
...
compositions (Theorem~4), SC-shared-variable compositions when all
shared accesses use sequentially consistent ordering (Theorem~5),
sound Owicki-Gries analysis for ordered shared-variable interactions
(Theorem~6),
and conservative rely-guarantee analysis for general shared-variable
interactions (Proposition~7).
Lean~4 mechanized proof sketches formalize Theorems~1, 4, 5, 6, and~7,
with clearly marked \texttt{sorry}'d subgoals delineating the verified
skeleton from remaining proof obligations.
%
A formal denotational semantics $D[\![\cdot]\!]$ maps every DSL
construct to trace sets.  Cross-validation against the operational
engine yields 428/432 (99.1\%) agreement; the 4~disagreements are
documented RISC-V asymmetric fence expressiveness gaps.
...
A formal Trusted Computing Base enumeration identifies 11 components
with explicit trust classifications (Verified, Trusted-Axiom,
Implementation-Trust), delineating which tool components carry
machine-checked guarantees and which require implementation trust.
%
A best-effort decomposition mode decomposes arbitrary concurrent code
beyond the pattern library into sub-patterns for approximate analysis.
We evaluate on 8 multi-pattern programs across 3 architectures.
An AST-based code analyzer matches source to patterns with
86.1\% exact-match accuracy on 238 benchmark snippets
(93.3\% top-3; Wilson CI $[81.0\%, 90.2\%]$), of which 35 are
independently sourced from 18 documented public repositories.
An independently constructed adversarial benchmark of 23 snippets
from 7~domains (cryptography, embedded systems, networking,
lock-free data structures, SIMD, coroutines, kernel code) achieves
13.0\% exact-match / 21.7\% top-3 accuracy, quantifying the
93\%$\to$13\% gap attributable to author-sampling bias and honestly
characterizing the AST analysis limitations on out-of-distribution code.
A CLI interface enables sub-second checking
in development workflows.

\litmus{} operates at pattern level---recognizing 137 known
...
A best-effort decomposition mode extends coverage to arbitrary
concurrent code by decomposing it into sub-patterns from
the library, though interactions between sub-patterns may be missed.
\end{abstract}
...
verifier: it matches code against 137 built-in litmus test patterns,
...
\item \textbf{Alethe proof certificates:} 1{,}370/1{,}370 independently
  checkable proof objects (1{,}095 UNSAT Alethe resolution proofs +
  275 self-certifying SAT models), upgrading from bare verdicts to
  machine-verifiable certificates (Section~\ref{sec:alethe-proofs}).
\item \textbf{Formal denotational semantics:} $D[\![\cdot]\!]$ mapping
  DSL constructs to trace sets, cross-validated against the operational
  engine at 428/432 (99.1\%) agreement
  (Section~\ref{sec:denotational-semantics}).
\item \textbf{Multi-layer validation:} 432/432 CPU cross-validation,
  94/94 GPU external cross-validation against published literature
  and specifications, and 170/171 DSL-to-\texttt{.cat}
  correspondence (Section~\ref{sec:validation},
  Section~\ref{sec:gpu-external},
  Section~\ref{sec:dsl-cat}).
\item \textbf{TCB analysis:} formal enumeration of 11 trust components
  with explicit Verified/Trusted-Axiom/Implementation-Trust
  classifications (Section~\ref{sec:tcb-analysis}).
\item \textbf{Lean~4 proof sketches:} mechanized proof skeletons for
  Theorems 1, 4, 5, 6, 7 with sorry'd subgoals delineating verified
  structure from remaining proof obligations.
\item \textbf{Honest evaluation:} adversarial benchmark of 23
  out-of-distribution snippets showing 13\% accuracy, quantifying
  the 93\%$\to$13\% author-sampling bias gap
  (Section~\ref{sec:adversarial-benchmark}).
\item \textbf{Severity-classified triage:} 423 data races, 82
  security vulnerabilities, and 130 benign artifacts among 635
  operationally-identified unsafe pairs
  (Section~\ref{sec:severity}).
\item \textbf{Code analysis on real code:} 86.1\% exact-match
  on 238 independently-documented snippets (93.3\% top-3;
  Wilson CI $[81.0\%, 90.2\%]$), including 35 independently sourced
  from 18 public repositories
...
matching against 137~built-in patterns via a weighted
...
\draw[arr] (src) -- (ast);
\draw[arr] (ast) -- (match);
\draw[arr] (match) -- node[right, font=\scriptsize]{137 patterns} (smt);
\draw[arr] (match) |- (enum);
\draw[arr] (enum) -- (cert);
\draw[arr] (smt) -- (fix);
...
matched against 137 litmus-test patterns, and checked by both enumeration
...
specifications (Section~\ref{sec:dsl-cat}).
...
engine at 428/432 (99.1\%) agreement (Section~\ref{sec:denotational-semantics}).
...
1{,}095 UNSAT (safety proofs with unsat cores) + 275 SAT (unsafety
witnesses with counterexample executions) = 1{,}370/1{,}370
(100\%), zero timeouts.
...
\begin{table}[ht]
\centering
\caption{Z3 certificate coverage: 1{,}370/1{,}370 pairs with extractable proofs.}
\label{tab:z3-certs}
\small
\begin{tabular}{lrrl}
\toprule
...
\midrule
UNSAT (safety proof)    & 1{,}095 & 5.7 assertions & No execution produces forbidden outcome \\
SAT (witness)    & 275 & --- & Concrete counterexample execution \\
Timeout           & 0   & --- & --- \\
\midrule
\textbf{Total}    & \textbf{1{,}370} & & \textbf{100\% coverage} \\
\bottomrule
\end{tabular}
\end{table}
...
\addplot[fill=green!50] coordinates {
  (x86,111) (SPARC,96) (ARM,80) (RISC-V,76)
  (CL-WG,122) (CL-Dev,122) (VK-WG,122) (VK-Dev,122)
  (CTA,122) (GPU,122)
};
\addplot[fill=red!40] coordinates {
  (x86,26) (SPARC,41) (ARM,57) (RISC-V,61)
  (CL-WG,15) (CL-Dev,15) (VK-WG,15) (VK-Dev,15)
  (CTA,15) (GPU,15)
};
...
identifies 735 safe + 635 unsafe pairs; the SMT encoding produces
1{,}095 UNSAT + 275 SAT. The 360 discrepancies are all GPU
...
For CPU models (432/432), the two methods agree exactly.
...
All 635 operationally-identified unsafe pairs (from the enumeration-based checker)
are classified by severity.  Of these, 275 are also confirmed by SMT SAT witnesses,
while the remaining 360 GPU-scope pairs are conservatively flagged by enumeration
but proven safe under the SMT axiomatic scope model:

\begin{table}[ht]
\centering
\caption{Severity classification of 635 unsafe pairs.}
\label{tab:severity}
\small
\begin{tabular}{lrrl}
\toprule
...
\midrule
Data race          & 423 & 66.6 & Stale/inconsistent reads \\
Security vuln.     & 82  & 12.9 & Broken mutual exclusion (Dekker, Peterson) \\
Benign             & 130 & 20.5 & Coherence artifacts (CoWR, CoWW, 2+2W) \\
\midrule
\textbf{Total}     & \textbf{635} & \textbf{100} & \\
\bottomrule
\end{tabular}
\end{table}

\noindent
The 82 security vulnerabilities demand immediate remediation; the
130 benign pairs can be deprioritized in triage.
The severity taxonomy is rule-based (pattern name $\to$ category),
calibrated against the following CWE entries:
\begin{itemize}[nosep]
\item \textbf{Data race (423):} CWE-362 (Race Condition), CWE-366
...
\item \textbf{Security vulnerability (82):} CWE-667 (Improper Locking),
  ...
  models.
\item \textbf{Benign (130):} No direct CWE mapping. Coherence-order
...
\begin{table}[ht]
\centering
\caption{Alethe proof certificate extraction: 1{,}370/1{,}370 pairs.}
...
\midrule
UNSAT $\to$ Alethe proof  & 1{,}095 & 100\% & Resolution proof DAG \\
SAT $\to$ verified model  & 275     & 100\% & Self-certifying substitution \\
Failed extractions        & 0       & 0\%   & --- \\
\midrule
\textbf{Total}            & \textbf{1{,}370} & \textbf{100\%} & \\
\bottomrule
\end{tabular}
\end{table}

\noindent
Alethe proofs average 109.3 steps (median~94, max~169) per certificate
...
Disagreement       & 4     & 0.9\% \\
\midrule
\textbf{Total}     & \textbf{432} & \\
\bottomrule
\end{tabular}
\end{table}

\noindent
All 4 disagreements occur on RISC-V patterns involving asymmetric
fence semantics (\texttt{mp\_fence\_wr}, \texttt{sb\_rel\_acq},
\texttt{sb\_dmb\_st\_only}, \texttt{sb\_dmb\_ld\_only}).  These
arise from a known expressiveness gap: the denotational model
...
the Cross-validated tier (denotational semantics) has 99.1\%
...
directly map to the 137 canonical litmus test templates.
...
\paragraph{GPU model.}
108/108 SMT internal consistency. The GPU model is additionally
...
custom model: 57, litmus round-trip: 137) plus 3{,}000 determinism
checks, all passing.

\paragraph{Performance.}
Full 1{,}370-pair analysis: mean 397\,ms (20 runs, 95\% bootstrap CI
$[310, 485]$\,ms). Individual 2-thread patterns: $<$1\,ms.

\subsection{GPU External Cross-Validation}
\label{sec:gpu-external}

To address the concern that 108/108 GPU internal consistency is
...
base'': while we do not provide full DRAT/LFSC/Alethe proof checking,
the exported SMT-LIB2 files enable \emph{cross-solver validation},
where a user can independently verify each certificate using an
alternative solver, reducing reliance on any single SMT solver's
correctness.
...
(137 patterns) but with per-thread fence recommendations, severity
...
Input scope & 137 patterns & litmus tests & full programs & full programs & full programs \\
Per-thread fences & \cmark & \xmark & \xmark & \cmark & \xmark \\
Sub-second & \cmark & \cmark & \xmark & \xmark & varies \\
Z3 certificates & 1{,}370/1{,}370 & \xmark & SMT-based & \xmark & \xmark \\
\bottomrule
\end{tabular}
\end{table}
...
The tool operates on 137 fixed patterns---including classical idioms,
RMW patterns, lock-free data structure patterns, release-acquire pairs,
N-thread generalizations, extended GPU scope variants, fence optimization
variants, multi-copy atomicity tests, and dependency chains---not
arbitrary programs.
It cannot discover novel concurrency bugs. The 137-pattern universe
...
coverage of the pattern set is unknown, though the expansion from
75 to 137 patterns significantly broadens the scope of recognized idioms.
...
is fast (median 189\,ms) and fully verifiable (1{,}370/1{,}370 Z3 certificates),
...
(137~patterns vs.\ full programs) but provides per-thread fence
...
108/108 SMT internal consistency. Additionally, 94/94 external
...
All 1{,}370 certificates depend on trusting Z3's internal proof search.
...
We have verified that all 275 SAT models satisfy the original
...
The 1{,}370/1{,}370 Alethe proof certificates provide independently
...
tool that analyzes 1{,}370 (pattern, architecture) pairs in median
189\,ms (mean 397\,ms).
Principal results:

\begin{itemize}[nosep]
\item \textbf{1{,}370/1{,}370 Alethe proof certificates} (complete enumeration):
  1{,}095 UNSAT safety proofs with Alethe resolution proofs (avg.\ 109.3 steps) +
  275 SAT unsafety witnesses with self-certifying models, zero timeouts,
  covering 137 patterns $\times$ 10 architectures.
\item \textbf{Denotational semantics} $D[\![\cdot]\!]$ cross-validated
  against operational engine: 428/432 (99.1\%) agreement on CPU patterns.
\item \textbf{432/432 CPU cross-validation} between enumeration and SMT;
...
\item \textbf{Severity classification:} 423 data races, 82
  security vulnerabilities, 130 benign (635 operationally-identified
  total unsafe pairs).
\item \textbf{170/171 DSL-to-\texttt{.cat} correspondence}
  across TSO, ARM, and RISC-V.
\item \textbf{Lean~4 proof sketches} for Theorems 1, 4, 5, 6, 7
  with mechanized proof skeletons and sorry'd subgoals.
\item \textbf{TCB analysis:} 11 components with explicit trust
  classifications (Verified, Trusted-Axiom, Implementation-Trust).
\item \textbf{Owicki-Gries compositional extension} for ordered
  shared-variable composition, with four composition strategies
  from exact (disjoint, SC-shared) to sound (Owicki-Gries, conservative RG).
\item \textbf{86.1\% exact-match, 93.3\% top-3} on 238
  independently-documented snippets; \textbf{13.0\% exact-match}
  on 23-snippet adversarial benchmark quantifying author-sampling bias.
...
pattern-level scope (137 patterns with no coverage audit against
real bug databases); best-effort decomposition may miss
...
not mechanized in a proof assistant. The 1{,}370/1{,}370 Z3 certificates
...
Theorems~\ref{thm:soundness}--\ref{thm:scope-fence} and
Theorems~\ref{thm:sc-composition}--\ref{thm:og-composition} are paper proofs.
Machine-checked evidence: 228/228 SMT + 1{,}370/1{,}370 Z3 certificates.
...
\noindent
All 1{,}370 SMT-LIB2 files are included in the artifact
...
\caption{Portability matrix (57 of 137 patterns shown; $\times$ 4 CPU + 6 GPU
configurations). Full 1{,}370-pair matrix available via \texttt{portcheck.py}. \cmark{} = Safe, \xmark{} = Unsafe.}
...
\item \textbf{Data race (423 pairs):} Standard weak-memory hazards.
  Most fixable with fences.
\item \textbf{Benign (130 pairs):} Coherence-order artifacts (CoWR, CoWW, 2+2W)
...
The universal catalog covers all 1{,}370 pairs with extractable proof
certificates:
1{,}095 UNSAT proofs (with Alethe proof certificates averaging 109.3 steps) and
275 SAT witnesses (with self-certifying rf/co assignments verified by substitution).
...
python3 portcheck.py --analyze-all            # Full 1370-pair analysis
...
For each of the 1{,}370 (pattern, architecture) pairs, we construct an
...
All 1{,}370 formulas complete with zero timeouts.  The median solving time
...
Each of the 635 unsafe (pattern, architecture) pairs is classified
...
\midrule
\textbf{All 1{,}370 pairs}    &  &  & 1{,}370 & 0.25 & 0.85 \\
\bottomrule
\end{tabular}
\end{table}
...
SMT encoding underspecified &
  ...
  Full QF\_LIA specification; 1{,}370 SMT-LIB2 files exported;
...
Theorems not mechanized &
  ...
  Alethe proofs for all 1{,}095 UNSAT verdicts \\
...
No SMT-LIB2 export &
  ...
  1{,}370 .smt2 files exported; independently verifiable \\
