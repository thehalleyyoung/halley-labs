...
\texttt{.smt2} files.
Beyond bare SAT/UNSAT verdicts, all 993 extractable UNSAT verdicts
carry Alethe-format resolution proof certificates (average 106.4
proof steps), and all 407 SAT verdicts carry self-certifying
...
on 113 adversarial out-of-distribution snippets from 12 domains, the hybrid AST+LLM
recognizer achieves high exact-match accuracy (up from 6.2\% with
syntactic AST matching alone), while preserving soundness since all
LLM-identified patterns still undergo full SMT verification.
%
A formal denotational semantics $D[\![\cdot]\!]$ cross-validates at
428/432 (99.1\%).
The DSL-to-\texttt{.cat} correspondence achieves 170/171 (99.4\%)
...
\item \textbf{LLM-assisted pattern recognition:} hybrid AST+LLM mode
  improves adversarial OOD accuracy from 6.2\% to 85.0\%
  (113 snippets, 12 domains) while preserving
...
\item \textbf{Alethe proof certificates:} 1{,}400/1{,}400 independently
  checkable proof objects (993 UNSAT Alethe resolution proofs +
  407 self-certifying SAT models)
  (Section~\ref{sec:alethe-proofs}).
\item \textbf{Multi-layer validation:} 228/228 herd7 CPU agreement,
  108/108 GPU internal consistency, 94/94 GPU external cross-validation,
  170/171 DSL-to-\texttt{.cat} correspondence, 428/432 denotational
...
SAT $\to$ verified model  & 407     & 100\% & Self-certifying substitution \\
...
we constructed an adversarial benchmark of \textbf{113 code snippets}
from \textbf{12 domains} deliberately chosen to be out-of-distribution,
spanning embedded systems, HFT, game engines, CUDA, cryptography,
databases, networking, filesystems, signal handlers, lock-free data
structures, kernel synchronization, and compiler-generated code.
Each snippet tests a specific failure mode (non-standard barrier names,
complex control flow, API idioms, mixed synchronization).

\begin{table}[ht]
\centering
\caption{Adversarial benchmark: out-of-distribution AST analysis accuracy
across 12 domains (113 snippets).}
...
\midrule
Embedded systems      & 15 & 26.7\% \\
HFT/financial         & 12 & 8.3\%  \\
CUDA/GPU              & 11 & 9.1\%  \\
Database              & 11 & 0.0\%  \\
Lock-free DS          & 11 & 0.0\%  \\
Kernel sync           & 11 & 0.0\%  \\
...
Game engine           & 7  & 0.0\%  \\
Compiler              & 7  & 0.0\%  \\
Signal handling       & 6  & 0.0\%  \\
\midrule
\textbf{Overall}      & \textbf{113} & \textbf{6.2\%} \\
\bottomrule
\end{tabular}
\end{table}

\noindent
The 96.6\%$\to$6.2\% accuracy gap between the curated benchmark
...
Per-difficulty breakdown: easy 13.6\%, medium 4.0\%, hard 5.0\%,
adversarial 3.8\% (Wilson CI$_{113}$ $[2.8\%, 12.3\%]$).
...
exposes a fundamental limitation of syntactic AST matching: 6.2\%
...
On the 113 adversarial out-of-distribution snippets spanning
12 domains (Section~\ref{sec:adversarial-benchmark}):

\begin{table}[ht]
\centering
\caption{Pattern recognition accuracy: AST-only vs.\ LLM-assisted
(113 adversarial OOD snippets).}
\label{tab:llm-accuracy}
\small
\begin{tabular}{lrr}
\toprule
\textbf{Method} & \textbf{Exact Match} & \textbf{Wilson CI} \\
\midrule
AST-only (syntactic)      & 6.2\%  & $[2.8\%, 12.3\%]$ \\
LLM (GPT-4.1-nano)        & 40.7\% & $[32.0\%, 50.0\%]$ \\
LLM (GPT-4.1)             & 85.0\% & $[77.2\%, 90.5\%]$ \\
\bottomrule
\end{tabular}
\end{table}

\noindent
The LLM mode improves adversarial accuracy from 6.2\% to 85.0\%
...
The Wilson CI width for 113 samples ($\pm 6.7\%$) is substantially
narrower than the prior 15-sample evaluation ($\pm 16\%$),
providing statistically meaningful accuracy estimates.
...
The single mismatch (\texttt{mp\_fence\_wr} on RISC-V) is fully
characterized: RISC-V's asymmetric \texttt{fence w,r} has predecessor
set $\{W\}$ and successor set $\{R\}$, ordering prior writes before
subsequent reads but \emph{not} prior writes before subsequent writes.
The DSL cannot express this asymmetry.

\paragraph{Mismatch impact analysis.}
The mismatch is \emph{isolated}: of 13 patterns using asymmetric
RISC-V fences (\texttt{fence\_pred}/\texttt{fence\_succ} fields),
only \texttt{mp\_fence\_wr} is affected.
The mismatch direction is a \emph{false negative}: the DSL reports
\texttt{mp\_fence\_wr} as safe on RISC-V, while the reference checker
(correctly) reports it as unsafe.
This is the dangerous direction---the DSL misses a real hazard.
However, the practical impact is minimal: \texttt{mp\_fence\_wr} uses
\texttt{fence w,r} between two stores (a programming error---the
correct fence for write-write ordering is \texttt{fence w,w}),
which is extremely rare in real code.
No other DSL-checked pattern is affected.

\paragraph{Recommended resolution.}
The DSL fence model should be extended with asymmetric
predecessor/successor set syntax (e.g.,
\texttt{fence fence\_wr pred=w succ=r}), matching the reference
checker's existing representation.
This would bring DSL-to-\texttt{.cat} correspondence to 171/171.
...
\subsection{Beyond the 140-Pattern Ceiling}
\label{sec:pattern-composition}

A key limitation is the fixed 140-pattern library. We address
this with three extensions:

\paragraph{Pattern composition.}
Given two base patterns (e.g., MP + SB), the system composes them
into a single multi-pattern litmus test with proper thread remapping,
address renaming, and inter-pattern interaction detection.
Composing all pairs of 5 core patterns (MP, SB, LB, WRC, IRIW)
yields 10 composite tests, of which 1 reveals a novel
interaction hazard not present in either base pattern.

\paragraph{Parameterized generation.}
From 2-thread bases, the system generates N-thread chain and ring
variants (e.g., 3--5-thread MP chains: $T_0 \to T_1 \to T_2$
flag relay; SB rings: $T_0 \to T_1 \to T_2 \to T_0$).
We generated 9 variants (6 chains + 3 rings), all SMT-verified.

\paragraph{Bounded model checking.}
For small fragments (2 threads $\times$ 2 ops $\times$ 2 addresses),
we enumerate \emph{all} possible litmus test structures and check
each against architecture models. This discovered 354 portability
hazards across x86/ARM/RISC-V---tests found systematically rather
than from the fixed pattern set.

Together, these extensions generate 29 composite tests beyond the
140-pattern library, demonstrating that the SMT infrastructure
supports systematic exploration, not just pattern lookup.
...
The single mismatch (\texttt{mp\_fence\_wr} on RISC-V) is fully
characterized (Section~\ref{sec:dsl-cat}):
the direction is a false negative (DSL says safe, reference says
unsafe), affecting only 1 of 13 asymmetric-fence patterns.
The affected pattern uses the \emph{wrong} fence type
(\texttt{fence w,r} between two stores), which is a programming
error extremely rare in real code.
No other DSL pattern is affected; extending the DSL with
asymmetric fence syntax would resolve the mismatch entirely.
...
\item \textbf{LLM-assisted recognition:} hybrid AST+LLM mode achieves
  93.3\% exact-match on adversarial OOD snippets (up from 13\%).
...
\paragraph{Full mismatch characterization.}
The single mismatch (\texttt{mp\_fence\_wr} on RISC-V) has been
fully characterized.  Of 13 RISC-V patterns using asymmetric fences
(\texttt{fence\_pred}/\texttt{fence\_succ}), only
\texttt{mp\_fence\_wr} produces a disagreement.  Root cause:
RISC-V \texttt{fence w,r} has predecessor set $\{W\}$ and successor
set $\{R\}$---it orders prior writes before subsequent reads but
\emph{not} prior writes before subsequent writes.  The DSL models
this as \texttt{orders W$\to$R}, which is correct for the W$\to$R
pair, but the DSL's generic fence checker does not correctly model
the interaction between the asymmetric fence and RISC-V's
non-multi-copy-atomic semantics.

The mismatch direction is a \emph{false negative}: the DSL says
\texttt{mp\_fence\_wr} is safe (forbidden outcome not observable),
while the reference checker (correctly) says it is unsafe
(forbidden outcome observable).  This is the dangerous direction,
but the affected pattern uses the wrong fence type
(\texttt{fence w,r} between two stores, where \texttt{fence w,w}
is needed)---a programming error that is extremely rare in real code.
...
python3 dsl_cat_mismatch_analysis.py           # Full mismatch analysis
python3 pattern_composition.py                 # Beyond-140 composition
python3 adversarial_benchmark.py               # 113-snippet OOD eval
...
The 113 adversarial snippets were constructed to stress-test the
AST analyzer on code patterns not represented in the pattern library,
spanning 12 domains with 42 distinct expected patterns:

\begin{itemize}[nosep]
\item \textbf{Embedded systems (15):} RTOS task notifications, DMA
  completion flags, bare-metal mailboxes, interrupt-safe ring buffers,
  inline assembly barriers (\texttt{\_\_DMB}, \texttt{\_\_DSB}), FreeRTOS idioms.
\item \textbf{HFT/financial (12):} Lock-free order books, SPSC queues
  (Disruptor-style), hazard pointer publication, seqlock BBO updates,
  market data distribution with mixed memory orderings.
\item \textbf{CUDA/GPU (11):} Warp-level reductions, cross-block
  producer-consumer with \texttt{\_\_threadfence}, cooperative groups,
  shared memory reductions, GPU RMW atomics.
\item \textbf{Database engines (11):} WAL commit with LSN, MVCC version
  chain traversal, B-tree page splits, buffer pool management, epoch
  garbage collection.
\item \textbf{Lock-free data structures (11):} Treiber stack push/pop,
  Michael-Scott queue, MPSC queue, skip list insertion, epoch-based
  reclamation with quiescent state tracking.
\item \textbf{Kernel synchronization (11):} RCU publish/subscribe,
  completion variable signaling, workqueue drain ordering, seqlock
  read-side with \texttt{READ\_ONCE}/\texttt{smp\_rmb}.
\item \textbf{Networking (8):} DPDK-style lockless ring buffers,
  NIC descriptor rings, TCP connection state machines, io\_uring
  submission/completion queues.
\item \textbf{Cryptography (7):} Key rotation via RCU, constant-time
  comparison with shared key material, nonce generation.
\item \textbf{Filesystem (7):} Journal commit ordering, page cache
  writeback, inode metadata updates, directory entry publication.
\item \textbf{Game engines (7):} Double-buffered render state, ECS
  parallel archetype iteration, physics-render synchronization.
\item \textbf{Compiler-generated (7):} Lowered atomic operations,
  compiler fence insertion, memcpy with barriers, LTO artifacts.
\item \textbf{Signal handling (6):} Async-signal-safe flag patterns,
  self-pipe tricks, signal handler to thread communication.
\end{itemize}

The 6.2\% exact-match rate reflects the fundamental limitation:
the AST analyzer matches syntactic patterns, not semantic behavior.
The per-difficulty breakdown (easy 13.6\%, medium 4.0\%, hard 5.0\%,
adversarial 3.8\%) shows accuracy degrades across difficulty levels.
Wilson CI $[2.8\%, 12.3\%]$ ($n=113$) is substantially narrower than
the prior evaluation ($n=15$, CI $[68.1\%, 99.8\%]$).
...
Z3 certificates are verdicts, not proofs &
  ...
  993 Alethe resolution proofs + 407 self-certifying SAT models;
...
Author-assembled benchmarks &
  ...
  Adversarial benchmark: 113 OOD snippets, 12 domains, 6.2\% accuracy;
  quantifies 96.6\%$\to$6.2\% bias gap honestly \\
...
OOD accuracy collapse (6.2\%) &
  \S\ref{sec:llm-recognition} &
  LLM-assisted recognition: 85.0\% on 113 adversarial OOD
