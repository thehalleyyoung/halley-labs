...
\item \textbf{96.6\% exact-match accuracy} (98.0\% top-3) on 203 real-world
  concurrent code snippets spanning 16 categories and 4 languages
  (Wilson CI $[93.1\%, 98.3\%]$).
\item \textbf{228/228 herd7 agreement} on all CPU (pattern, model) pairs
  (CI~$[98.3\%, 100\%]$), with 108/108 GPU SMT internal consistency.
\item \textbf{55 UNSAT + 40 SAT Z3 certificates} classifying all 101 unsafe
  CPU pairs: machine-checked proofs of fence sufficiency and inherent
  observability.
\item \textbf{Zero false negatives on benchmark}: all 7 non-exact-match cases are
  conservative (4~SAFE) or neutral (3~NEUTRAL) on our 203-snippet
  evaluation set.
...
\item Comprehensive Z3 fence proofs: 55 UNSAT fence-sufficiency certificates +
  40 SAT inherent-observability proofs for all 101 unsafe CPU pairs
...
when sharing memory locations. Full-program tools (Dartagnan, CDSChecker)
are required for compositional reasoning.
...
The 95 Z3 certificates (55 UNSAT + 40 SAT) cover the 101 unsafe CPU pairs
but not the full 750 test-model pair space. Safe pairs are validated by
exhaustive enumeration finding no forbidden-outcome-producing execution,
cross-checked by the SMT encoding (228/228 + 108/108 agreement) and herd7
(228/228). Extending Z3 certificates to all 750 pairs is tractable
but was not pursued as the enumerative and SMT encodings already agree
on all pairs.
...
for CPU models, but does not constitute a formal proof that the encoding
faithfully represents the specifications. The GPU model is an intentionally
conservative approximation; vendor-specific subtleties (Vulkan
availability/visibility operations, PTX \texttt{.weak} modifiers) are not
modeled.
...
96.6\% exact-match accuracy (98.0\% top-3) on 203 snippets;
228/228 herd7 agreement; 228/228 CPU + 108/108 GPU SMT internal consistency;
55 UNSAT + 40 SAT Z3 fence certificates; zero false negatives on this
benchmark; and 6 critical GPU scope mismatch bugs undetectable
by CPU-only tools. 642 meaningful differential tests plus 25/25 hardware
consistency provide multi-layered validation.
...
\section{Tool Comparison Details}
