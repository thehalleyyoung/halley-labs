...
on 15 adversarial out-of-distribution snippets, the hybrid AST+LLM
recognizer achieves 93.3\% exact-match accuracy (up from 13\% with
...
\item \textbf{LLM-assisted pattern recognition:} hybrid AST+LLM mode
  improves adversarial OOD accuracy from 13\% to 93.3\% while preserving
...
On 15 adversarial out-of-distribution snippets spanning
cryptography, embedded systems, networking, lock-free data structures,
GPU kernels, coroutines, and kernel code:
...
\begin{tabular}{lrr}
\toprule
\textbf{Method} & \textbf{Exact Match} & \textbf{Top-3} \\
\midrule
AST-only (syntactic)      & 13.0\% & 21.7\% \\
LLM (GPT-4.1-nano)        & 40.0\% & 46.7\% \\
LLM (GPT-4.1)             & 93.3\% & 93.3\% \\
\bottomrule
\end{tabular}
\end{table}

\noindent
The LLM mode improves adversarial accuracy from 13\% to 93\%
