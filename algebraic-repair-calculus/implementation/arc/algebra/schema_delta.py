...
class ChangeType(SchemaOperation):
    ...
    def apply(self, schema: Schema) -> Schema:
        ...
        if col is not None:
            # Auto-populate coercion_expr when not provided but a safe coercion exists
            if self.coercion_expr is None:
                auto_expr = TYPE_COMPATIBILITY_MAP.get((self.old_type, self.new_type))
                if auto_expr is not None:
                    object.__setattr__(self, "coercion_expr", auto_expr)
    ...
    def is_safe(self) -> bool:
        """Check if this type change is a known safe (lossless) coercion."""
        return (self.old_type, self.new_type) in TYPE_COMPATIBILITY_MAP

    @classmethod
    def from_type_pair(
        cls, column_name: str, old_type: SQLType, new_type: SQLType
    ) -> ChangeType:
        """Create a ChangeType with auto-populated coercion expression."""
        coercion = TYPE_COMPATIBILITY_MAP.get((old_type, new_type))
        return cls(
            column_name=column_name,
            old_type=old_type,
            new_type=new_type,
            coercion_expr=coercion,
        )
...
# ---------------------------------------------------------------------------
# Type compatibility map and coercion helpers
# ---------------------------------------------------------------------------

TYPE_COMPATIBILITY_MAP: Dict[Tuple[SQLType, SQLType], str] = {
    # Widening numeric
    (SQLType.INTEGER, SQLType.BIGINT): "CAST({col} AS BIGINT)",
    (SQLType.SMALLINT, SQLType.INTEGER): "CAST({col} AS INTEGER)",
    (SQLType.SMALLINT, SQLType.BIGINT): "CAST({col} AS BIGINT)",
    (SQLType.FLOAT, SQLType.DOUBLE): "CAST({col} AS DOUBLE PRECISION)",
    (SQLType.FLOAT, SQLType.DECIMAL): "CAST({col} AS DECIMAL)",
    (SQLType.DOUBLE, SQLType.DECIMAL): "CAST({col} AS DECIMAL)",
    # String widening
    (SQLType.VARCHAR, SQLType.TEXT): "CAST({col} AS TEXT)",
    (SQLType.CHAR, SQLType.VARCHAR): "CAST({col} AS VARCHAR)",
    (SQLType.CHAR, SQLType.TEXT): "CAST({col} AS TEXT)",
    # Temporal widening
    (SQLType.DATE, SQLType.TIMESTAMP): "CAST({col} AS TIMESTAMP)",
    (SQLType.TIMESTAMP, SQLType.TIMESTAMPTZ): "CAST({col} AS TIMESTAMPTZ)",
    (SQLType.DATE, SQLType.TIMESTAMPTZ): "CAST({col} AS TIMESTAMPTZ)",
    # JSON
    (SQLType.JSON, SQLType.JSONB): "CAST({col} AS JSONB)",
    (SQLType.TEXT, SQLType.JSON): "CAST({col} AS JSON)",
    (SQLType.TEXT, SQLType.JSONB): "CAST({col} AS JSONB)",
    # Boolean
    (SQLType.INTEGER, SQLType.BOOLEAN): "CAST({col} AS BOOLEAN)",
    (SQLType.BOOLEAN, SQLType.INTEGER): "CAST({col} AS INTEGER)",
}


def is_safe_coercion(old_type: SQLType, new_type: SQLType) -> bool:
    """Check if a type conversion is a known safe (lossless) coercion."""
    return (old_type, new_type) in TYPE_COMPATIBILITY_MAP


def get_coercion_expr(old_type: SQLType, new_type: SQLType) -> Optional[str]:
    """Return the SQL CAST expression for a safe coercion, or None."""
    return TYPE_COMPATIBILITY_MAP.get((old_type, new_type))
