...
def _adapt_graph(graph):
    """Adapt graph.pipeline.PipelineGraph to the types.base interface if needed."""
    if hasattr(graph, 'is_acyclic'):
        return graph
    # Wrap arc.graph.pipeline.PipelineGraph
    class _Adapter:
        def __init__(self, g):
            self._g = g
        def is_acyclic(self):
        ...
        def parents(self, nid):
            return self._g.predecessors(nid)
        def children(self, nid):
            return self._g.successors(nid)
        def __getattr__(self, name):
            return getattr(self._g, name)
    return _Adapter(graph)
            ...
            if isinstance(node.operator_config, JoinConfig):
                ...
                if len(parents) >= 2:
                    if parent_id == parents[0]:
                        ...
                        # Multi-way join: iterate remaining parents to find match
                        for idx in range(2, len(parents)):
                            if parent_id == parents[idx]:
                                if affected_cols & all_join_keys:
                                    return delta
                                # Non-key columns still propagate
                                return delta
                    # Parent not matched to any join key set — still propagate
                    # as it may affect non-key columns passed through the join
                    return delta

        # CTE / subquery pass-through: CUSTOM operators (CTEs, subqueries)
        # propagate deltas unless a projection explicitly blocks them.
        if node.operator == SQLOperator.CUSTOM:
            if isinstance(node.operator_config, SelectConfig):
                kept = set(node.operator_config.columns)
                if affected_cols and not affected_cols & kept:
                    return CompoundPerturbation()
            # No explicit projection — delta passes through the CTE
            return delta
