...
comprises \textbf{45} Python files (\(\sim\!\)\textbf{8,500} lines), backed by \textbf{127}
...
\item A full implementation (\textbf{[TBD]} files, $\sim$\textbf{[TBD]} lines, \textbf{[TBD]} tests)
  with an evaluation showing \textbf{[TBD]}$\times$ improvement over five SOTA
...
and diamond topologies achieve \textbf{\textbf{[TBD]} annihilation} --- every
...
\textbf{\textbf{[TBD]} node annihilation} for schema-only perturbations:
...
We evaluate 8 pipeline topologies: \textsc{Linear} (chain),
\textsc{Diamond} (fork-join), \textsc{Star} (hub-and-spoke),
\textsc{Tree} (fan-out), \textsc{InvTree} (fan-in),
\textsc{Grid} (lattice), \textsc{Bipartite} (two-layer),
and \textsc{Random} (Erd\H{o}s--R\'enyi DAG).
...
Each configuration is run with 5 random seeds.  With 8 topologies
$\times$ 4 perturbation types $\times$ 5 seeds = \textbf{[TBD]} end-to-end tests.

\subsection{RQ1: Correctness}

\begin{table}[t]
\caption{Correctness results: \textbf{[TBD]} tests pass across all topologies
...
Star      & \cmark & \cmark & \cmark & \cmark \\
Tree      & \cmark & \cmark & \cmark & \cmark \\
InvTree   & \cmark & \cmark & \cmark & \cmark \\
Grid      & \cmark & \cmark & \cmark & \cmark \\
Bipartite & \cmark & \cmark & \cmark & \cmark \\
Random    & \cmark & \cmark & \cmark & \cmark \\
\midrule
\textbf{Total} & \multicolumn{4}{c}{\textbf{[TBD]}} \\
\bottomrule
\end{tabular}\\[4pt]
{\footnotesize \cmark{} = all 5 seeds pass (repair output $\equiv$
recompute output).}
\end{table}

Across all \textbf{[TBD]} test configurations, \arc produces output \emph{bit-identical}
...
\caption{Mean cost ratio relative to full recomputation (lower is better)
  across 8 pipeline topologies.  ARC is the \emph{only} system that
...
\textbf{Pert.\ Type} &
  ...
  \textbf{Mater.} \\
\midrule
Schema only   & \textbf{[TBD]} & \textbf{[TBD]} & \textbf{[TBD]} & \textbf{[TBD]} & \textbf{[TBD]} & \textbf{[TBD]} \\
Data (small)  & \textbf{[TBD]} & \textbf{[TBD]} & \textbf{[TBD]} & \textbf{[TBD]} & \textbf{[TBD]} & \textbf{[TBD]} \\
Data (500 rows) & \textbf{[TBD]} & \textbf{[TBD]} & \textbf{[TBD]} & \textbf{[TBD]} & \textbf{[TBD]} & \textbf{[TBD]} \\
Quality only  & \textbf{[TBD]} & \textbf{[TBD]} & \textbf{[TBD]} & \textbf{[TBD]} & \textbf{[TBD]} & \textbf{[TBD]} \\
Compound      & \textbf{[TBD]} & \textbf{[TBD]} & \textbf{[TBD]} & \textbf{[TBD]} & \textbf{[TBD]} & \textbf{[TBD]} \\
\bottomrule
\end{tabular}\\[4pt]
...
\item \textbf{Schema/Quality:} \arc achieves \emph{zero} cost via algebraic
  annihilation; all baselines require full recomputation ($\geq$\,\textbf{[TBD]}).
\item \textbf{Data (small $\delta$):} \arc is competitive with DBSP and
  Materialize (all $\leq$\,\textbf{[TBD]} of full cost), and \textbf{[TBD]}$\times$ faster
  than dbt (which materialises each model).
\item \textbf{Data (large $\delta$, 500 rows):} \arc achieves \textbf{[TBD]} vs.\
  DBSP's \textbf{[TBD]} --- a \textbf{[TBD]}$\times$ advantage from the DP planner selecting
  optimal repair strategies per node.
\item \textbf{Compound:} The most differentiating case. All baselines are
  dominated by the schema component ($\geq$\,\textbf{[TBD]}); \arc decomposes the
  perturbation, annihilates the schema part, and repairs only the data
  delta (\textbf{[TBD]}), giving a \textbf{\textbf{[TBD]}$\times$ improvement}.
\end{itemize}
...
\midrule
5       & \textbf{[TBD]}    & \textbf{[TBD]}   & \textbf{[TBD]} \\
10      & \textbf{[TBD]}    & \textbf{[TBD]}   & \textbf{[TBD]} \\
50      & \textbf{[TBD]}    & \textbf{[TBD]}   & \textbf{[TBD]} \\
100     & \textbf{[TBD]}    & \textbf{[TBD]}   & \textbf{[TBD]} \\
500     & \textbf{[TBD]}    & \textbf{[TBD]}   & \textbf{[TBD]} \\
1\,000  & \textbf{[TBD]}    & \textbf{[TBD]}   & \textbf{[TBD]} \\
\bottomrule
\end{tabular}
\end{table}
...
% \end{loglogaxis}
% \end{tikzpicture}
\caption{DP planner scaling.  Empirical exponent is \textbf{[TBD]} (near-linear),
...
exponent of \textbf{\textbf{[TBD]}}, confirming near-linear scaling.  Pipeline
construction throughput remains stable at \textbf{\textbf{[TBD]} nodes/ms} across
all sizes.  The DP planner completes in \textbf{\textbf{[TBD]}\,ms} for 10-node
pipelines and \textbf{\textbf{[TBD]}\,ms} for 1\,000-node pipelines.
...
\textbf{Nodes} & \textbf{DP (ms)} & \textbf{LP (ms)} &
  \textbf{Speedup} \\
\midrule
10   & \textbf{[TBD]}  & \textbf{[TBD]}   & \textbf{[TBD]}$\times$ \\
50   & \textbf{[TBD]}  & \textbf{[TBD]}   & \textbf{[TBD]}$\times$ \\
100  & \textbf{[TBD]}  & \textbf{[TBD]}   & \textbf{[TBD]}$\times$ \\
\bottomrule
\end{tabular}
\end{table}

The DP planner is up to \textbf{\textbf{[TBD]}$\times$} faster than the LP planner
for small pipelines
...
\textbf{Topology} & \textbf{Schema} & \textbf{Data} &
  \textbf{Quality} & \textbf{Mixed} \\
\midrule
Linear    & \textbf{[TBD]}   & \textbf{[TBD]}  & \textbf{[TBD]}  & \textbf{[TBD]} \\
Diamond   & \textbf{[TBD]}   & \textbf{[TBD]}  & \textbf{[TBD]}  & \textbf{[TBD]} \\
Star      & \textbf{[TBD]}    & \textbf{[TBD]}  & \textbf{[TBD]}  & \textbf{[TBD]} \\
Tree      & \textbf{[TBD]}    & \textbf{[TBD]}  & \textbf{[TBD]}  & \textbf{[TBD]} \\
InvTree   & \textbf{[TBD]}    & \textbf{[TBD]}  & \textbf{[TBD]}  & \textbf{[TBD]} \\
Grid      & \textbf{[TBD]}    & \textbf{[TBD]}  & \textbf{[TBD]}  & \textbf{[TBD]} \\
Bipartite & \textbf{[TBD]}    & \textbf{[TBD]}  & \textbf{[TBD]}  & \textbf{[TBD]} \\
...
with \textbf{\textbf{[TBD]}} in linear and diamond topologies.  This occurs because
...
Data perturbations show lower but still significant annihilation
(\textbf{[TBD]}), driven by data deltas that cancel through aggregation
nodes (e.g., a row insertion that does not change a \texttt{COUNT}).
...
\midrule
Delta composition ($\compose$)    & $<$\textbf{[TBD]}\,ms \\
Delta inversion (${}^{-1}$)       & $<$\textbf{[TBD]}\,ms \\
Severity computation ($\severity$) & $<$\textbf{[TBD]}\,ms \\
Push-forward ($\push$)             & $<$\textbf{[TBD]}\,ms \\
Annihilation check ($\annihilate$) & $<$\textbf{[TBD]}\,ms \\
Identity test                      & $<$\textbf{[TBD]}\,ms \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:micro} confirms that all algebra operations complete in
sub-millisecond time.  Composition takes $<$\textbf{[TBD]}\,ms and severity
$<$\textbf{[TBD]}\,ms, ensuring that the algebraic overhead is negligible
...
\midrule
Unit tests (algebra, types, graph)    & \textbf{[TBD]} \\
Integration tests (end-to-end repair) & \textbf{[TBD]} \\
Property-based tests (Hypothesis)     & \textbf{[TBD]} \\
\midrule
...
All \textbf{[TBD]} tests pass on Python 3.11+ with DuckDB 0.9+.
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}lrrrl@{}}
    ...
    \midrule
    Jaffle Shop (dbt)      &   \textbf{[TBD]} nodes &  \textbf{[TBD]} & \textbf{[TBD]} & \cmark\ all verified \\
    Sentry Migrations      & \textbf{[TBD]} ops &   \textbf{[TBD]} & \textbf{[TBD]} & \cmark\ all verified \\
    \bottomrule
  \end{tabular}
\end{table}

\paragraph{Jaffle Shop.}
We parsed the \textbf{[TBD]} SQL models and their \texttt{ref()} dependencies from
...
repair plan achieves \textbf{\textbf{[TBD]} cost savings} versus na\"{i}ve
...
repository, comprising \textbf{[TBD]} individual operations
...
are modest at \textbf{\textbf{[TBD]}}: individual migrations are already
...
leaves without cancellation.  Mixed perturbations in random graphs also
show lower annihilation (\textbf{[TBD]}) because the interaction between schema
...
the 8 topology classes cover common industrial patterns, production
...
DP/LP planner selects cost-optimal repair strategies.  Across \textbf{[TBD]}
end-to-end tests, \arc achieves \textbf{[TBD]} correctness and outperforms five
...
$\textbf{[TBD]}\times$ improvement for compound perturbations.  For data-only
changes, \arc is competitive with specialized IVM engines.  The engine
scales near-linearly (exponent \textbf{[TBD]}) to \textbf{[TBD]}-node pipelines.
