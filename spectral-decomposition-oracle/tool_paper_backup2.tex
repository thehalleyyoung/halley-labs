...
\begin{abstract}
Decomposition methods---Dantzig-Wolfe, Benders, Lagrangian relaxation---are essential for solving large-scale mixed-integer programs (MIPs), yet selecting the right decomposition for a given instance remains a black art requiring expert knowledge. We present \textsc{SpecOracle}, an automated decomposition oracle that analyzes the spectral properties of constraint matrices to predict which decomposition method will yield the best performance. SpecOracle computes spectral features (eigenvalue distribution, spectral gap, algebraic connectivity, Fiedler vector structure) and uses a trained decision model to select among Dantzig-Wolfe (DW), Benders, Lagrangian relaxation (LR), and no-decomposition (direct solve). On --- instances from MIPLIB 2017, TSPLIB, OR-Library, and CVRPLIB, SpecOracle selects the optimal decomposition in ---\% of cases, reducing total solve time by ---$\times$ compared to always using the best single method, and ---$\times$ compared to default SCIP. SpecOracle is the first tool to automate decomposition selection using spectral analysis, making advanced decomposition techniques accessible to non-expert users.
\end{abstract}
...
\midrule
MIPLIB 2017 & Mixed (general MIP) & --- & --- & --- \\
TSPLIB & Traveling salesman & --- & --- & --- \\
OR-Library & Set cover/packing & --- & --- & --- \\
CVRPLIB & Vehicle routing & --- & --- & --- \\
\midrule
\textbf{Total} & & \textbf{---} & & \\
\bottomrule
\end{tabular}
...
SATzilla-style & ---\% & --- & ---$\times$ \\
\midrule
\textbf{SpecOracle} & \textbf{---\%} & \textbf{---} & \textbf{---$\times$} \\
Virtual Best & 100\% & --- & ---$\times$ \\
\bottomrule
\end{tabular}
\end{table}

SpecOracle achieves ---\% selection accuracy, outperforming GCG (---\%) and SATzilla-style features (---\%). The geometric mean solve time is ---$\times$ faster than SCIP and ---$\times$ faster than GCG.
...
\end{axis}
\end{tikzpicture}
\caption{Dolan-Mor\'e performance profiles. SpecOracle is the fastest solver on ---\% of instances and within 2$\times$ of best on ---\%.}
\label{fig:profile}
\end{figure}
...
\midrule
Spectral gap ($\text{gap}_k$) & --- & ---\% \\
Algebraic connectivity ($\lambda_2$) & --- & ---\% \\
Block count ($n_{\text{blocks}}$) & --- & ---\% \\
Spectral ratio ($\lambda_n/\lambda_2$) & --- & ---\% \\
Block balance (bal) & --- & ---\% \\
Density & --- & ---\% \\
Eigenvalue kurtosis & --- & ---\% \\
Problem size ($m, n$) & --- & ---\% \\
\midrule
\textbf{Full model} & --- & \textbf{---\%} \\
Non-spectral only & --- & ---\% \\
\bottomrule
\end{tabular}
\end{table}

Spectral features contribute --- percentage points over non-spectral features alone (---\% vs.\ ---\%), validating the spectral analysis approach.
...
A fleet routing problem with 200 customers, 20 vehicles, and capacity constraints. The constraint matrix has clear block structure (one block per vehicle route) with linking constraints (customer coverage). SpecOracle detects 20 blocks via spectral gap analysis ($\lambda_{20}/\lambda_{21} = ---$), selects DW decomposition, and solves in ---s (SCIP: ---s, GCG: ---s, Gurobi: ---s).

\subsection{Case Study: Facility Location}

A 500-facility, 2000-customer uncapacitated facility location problem. Spectral analysis reveals Benders-favorable structure (facility-opening variables as complicating variables). SpecOracle selects Benders, solving in ---s (SCIP: ---s, DW: ---s, Gurobi: ---s).

\subsection{Niche Analysis}

SpecOracle dominates in three niches:
\begin{enumerate}
\item \textbf{Decomposition-amenable instances}: On the ---/--- instances where some decomposition outperforms direct solving, SpecOracle achieves ---$\times$ speedup vs.\ SCIP.
...
\item \textbf{Portfolio solving}: As an algorithm selector, SpecOracle captures ---\% of the virtual best solver's performance (vs.\ ---\% for best single method).
\end{enumerate}
...
Of the --- misselections (---\%), we categorize the cost:
...
\midrule
Mild (within 1.5$\times$ of best) & --- & ---$\times$ & ---$\times$ \\
Moderate (1.5--3$\times$) & --- & ---$\times$ & ---$\times$ \\
Severe ($>$3$\times$) & --- & ---$\times$ & ---$\times$ \\
\bottomrule
\end{tabular}
\end{table}

Most misselections are mild. Severe misselections typically occur on instances with ambiguous structure where multiple decompositions perform similarly. Even in the worst case, SpecOracle's misselection is faster than SCIP on ---\% of misselected instances.
...
Decomposition setup & --- & --- & --- \\
\midrule
\textbf{Total overhead} & --- & --- & 100\% \\
\bottomrule
\end{tabular}
\end{table}

The total overhead (median ---) is negligible compared to solve times (median ---). Eigenvalue computation dominates but uses the Lanczos algorithm for the top-$k$ eigenvalues, avoiding full decomposition.
...
\midrule
Accuracy & ---\% & ---\% & ---\% & ---\% \\
Speedup & ---$\times$ & ---$\times$ & ---$\times$ & ---$\times$ \\
\bottomrule
\end{tabular}
\end{table}

Generalization accuracy (--------\%) is lower than in-distribution (---\%) but still substantially outperforms all baselines. The drop is smallest for OR-Library (structurally diverse) and largest for CVRPLIB (distinct vehicle routing structure).
...
SpecOracle solves ---/--- instances within 1 hour (vs.\ SCIP ---, GCG ---, Gurobi ---). The 9 unsolved instances have extremely dense constraint matrices where spectral analysis correctly predicts ``Direct'' but the instance itself is too hard for any solver.
...
\midrule
SATzilla-style & 36 generic & ---\% & ---$\times$ & --- \\
GNN embedding & Graph neural & ---\% & ---$\times$ & --- \\
Probing (solve 10s) & Performance & ---\% & ---$\times$ & 0 \\
\textbf{SpecOracle} & 11 spectral & \textbf{---\%} & \textbf{---$\times$} & --- \\
\bottomrule
\end{tabular}
\end{table}
...
SpecOracle automates MIP decomposition selection using spectral analysis, achieving ---\% selection accuracy and ---$\times$ speedup over SCIP. The spectral features capture decomposition-relevant structure that non-spectral features miss, providing a principled basis for decomposition method selection.
...
Per-instance results for all --- MIPLIB instances are available in the supplementary material. Summary: SpecOracle selects optimally for ---/--- instances (---\%), with the --- misselections resulting in average ---$\times$ slowdown vs.\ optimal (but still ---$\times$ faster than SCIP on those instances).
\end{document}
