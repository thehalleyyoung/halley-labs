...
(1)~complex multi-hop attribute access (e.g., \texttt{self.model.encoder});
(2)~dynamic reshape with fully symbolic dimensions;
(3)~loop-based layer application with data-dependent indexing.
These are engineering limitations, not fundamental to the approach.
The graph extractor now supports \texttt{ConvTranspose2d},
\texttt{F.interpolate}, element-wise multiply (\texttt{x * y}),
and \texttt{view(x.size(0), -1)} flattening patterns, which
previously caused false positives on architectures like U-Net,
SE-Net, ResNet skip connections, and BERT.
...
Unsupported operations may cause
...
functions are modeled.  The graph extractor now covers
\texttt{nn.ConvTranspose2d}, \texttt{F.interpolate}, element-wise
multiply/subtract, \texttt{nn.ModuleList}, and common
\texttt{view(x.size(0), -1)} flattening patterns.

\paragraph{Mechanized proofs.}
The core soundness theorems are mechanized in Lean~4
(\texttt{lean/TheoryCombination.lean}), including:
theory combination soundness (Theorem~\ref{thm:combination}),
broadcast commutativity and associativity,
non-tautological propagator soundness (connecting the executable
boolean checker to the semantic specification via
\texttt{broadcastDim\_sound}),
CEGAR convergence from finite predicate universe,
and the forward direction of the PARTITION reduction for
NP-completeness.
\end{proof}
