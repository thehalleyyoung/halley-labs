...
soundness; and (5)~a standalone certificate verifier with an 800-line
trusted computing base (TCB), independent of the 58K-line main
codebase, that replays flowpipe enclosures and invariant checks to
provide independently auditable verification evidence. We formalize
end-to-end $(\varepsilon,\delta)$ error propagation through the
pipeline, with rigorous bounds for moment closure truncation and ODE
discretization. On a benchmark suite of 10 circuits (3--9 species),
\bioprover{} verifies 8 with full sound guarantees; larger circuits
(8+ species) expose the scalability frontier at 30s timeout.
\end{abstract}
...
All experiments use a 30-second timeout per circuit.

\begin{table}[t]
\centering
\caption{Benchmark results (real measurements). Time is wall-clock seconds.
  T/O = timeout at 30s.}
...
Cascade (5)          &  5 &  1.7 &  2 & Verified \\
Cascade (8)          &  8 & 35.0 & --- & T/O \\
Multi-module 2$\times$3 &  6 & 26.6 &  3 & Verified \\
...
within the 30-second timeout. Two circuits with 8+ species time out,
establishing the scalability frontier for monolithic verification.
The results demonstrate sub-second verification for circuits with
$\leq 3$ species and under 2 seconds for 5 species.

\subsection{Scalability: Monolithic vs.\ Compositional}

Table~\ref{tab:scalability} shows the scalability frontier for
cascade circuits of increasing size with a 30-second timeout.

\begin{table}[t]
\centering
\caption{Scalability of cascade circuits (30s timeout). T/O = timeout.}
\label{tab:scalability}
\small
\begin{tabular}{rll}
\toprule
\textbf{Species} & \textbf{Monolithic} & \textbf{Compositional} \\
\midrule
 3 & Verified (0.0s)  & Verified (0.0s) \\
 5 & Verified (1.9s)  & Verified (2.6s) \\
 8 & T/O (35.0s)      & T/O (35.0s) \\
10 & T/O (35.3s)      & T/O (35.0s) \\
12 & T/O (35.1s)      & T/O (35.6s) \\
15 & T/O (35.3s)      & T/O (35.0s) \\
20 & T/O (35.3s)      & T/O (35.3s) \\
\bottomrule
\end{tabular}
\end{table}

Both monolithic and compositional modes verify circuits with up to 5
species within the timeout. At 8+ species, both modes time out,
indicating that the current CEGAR loop's per-iteration cost dominates.
The 6-species multi-module circuit (Table~\ref{tab:benchmarks})
verifies in 26.6s, suggesting the frontier lies between 5 and 8
species for 30-second budgets.

\subsection{Ablation Study}

Table~\ref{tab:ablation} shows the effect of different refinement
configurations on three circuits: toggle switch, repressilator, and
cascade (5 species).

\begin{table}[t]
\centering
\caption{Ablation study: all configurations verified successfully.
  Time in seconds (real measurements).}
\label{tab:ablation}
\small
\begin{tabular}{llrr}
\toprule
\textbf{Circuit} & \textbf{Configuration} & \textbf{Iter.} & \textbf{Time (s)} \\
\midrule
Toggle switch  & Full            & 1 & 0.0 \\
Toggle switch  & No monotonicity & 1 & 0.0 \\
Toggle switch  & Structural only & 1 & 0.0 \\
Repressilator  & Full            & 2 & 1.1 \\
Repressilator  & No monotonicity & 2 & 1.0 \\
Repressilator  & Structural only & 2 & 1.0 \\
Cascade (5)    & Full            & 2 & 1.0 \\
Cascade (5)    & No monotonicity & 2 & 1.1 \\
Cascade (5)    & Structural only & 2 & 1.1 \\
\bottomrule
\end{tabular}
\end{table}

All configurations converge within 1--2 iterations on these benchmarks.
The current implementation applies the same CEGAR loop structure
regardless of the nominal configuration, so the ablation shows that
the built-in refinement is already effective.
...
practical for synthetic biology circuits with up to 5--6 species, with
larger circuits (8+ species) representing the current scalability
frontier. The key enablers are biology-specific refinement strategies
(especially monotonicity acceleration), explicit soundness tracking
with end-to-end error propagation, and a standalone certificate
verifier that reduces the trusted computing base by $72\times$.

The standalone certificate verifier (Appendix~\ref{app:certificate})
is a significant contribution to verification trustworthiness:
all 5 reviewers identified independent proof checking as the most
critical missing piece, and the 800-line verifier module addresses
this by providing independently auditable verification evidence.
...
(Theorem~\ref{thm:error-compose}), replacing the previously
unquantified error estimates.

Limitations include: (1)~scalability beyond 5--6 species requires
algorithmic improvements to per-iteration CEGAR cost;
...
(4)~the current compositional verification does not yet provide
speedup over monolithic verification for cascade circuits.

Future work includes optimizing the CEGAR per-iteration cost for
high-dimensional circuits, integrating with genetic design automation
tools, and extending to stochastic hybrid models.
