...
\emph{Second}, a compositional variance propagation engine analyses arbitrary \texttt{nn.Module} computation graphs via typed rules (linear, convolution, activation, normalization, residual, attention, pooling, dropout), achieving 83\% phase agreement across 18 architectures spanning 6 families.
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Arbitrary computation graph analysis} (\S\ref{sec:arbitrary-graphs}): A compositional mean-field engine that decomposes any \texttt{nn.Module} into typed variance propagation rules. Validated on 18 architectures across 6 families with 83\% phase agreement.
...
\begin{table}[t]
\centering
\caption{Compositional variance propagation rules.}
\label{tab:rules}
\small
\begin{tabular}{@{}llc@{}}
\toprule
...
\textsc{Act.}($\phi$) & $V_\phi(q) = \EE[\phi(\sqrt{q}\,z)^2]$ & $V'_\phi(q)$ \\
\textsc{Norm}($\gamma$) & $\gamma^2$ (reset) & $0$ \\
\textsc{Resid.}($\alpha$) & $q_{\mathrm{skip}} + \alpha\, q_{\mathrm{br.}}$ & $1 + \alpha$ \\
\textsc{Attn.} & $\sigma_w^4 q\, c(\sigma_w^4 q^2)$ & $\sigma_w^4$ \\
\textsc{Drop.}($p$) & $q/(1\!-\!p)$ or $q$ & $1/(1\!-\!p)$ \\
\textsc{AvgP.}($k$) & $q/k$ & $1/k$ \\
\textsc{MaxP.}($k$) & $q(1 + 2\ln k/\pi)$ & see text \\
\bottomrule
\end{tabular}
\end{table}
...
The engine correctly classifies all 3 Transformers (Pre-LN, 2--6~layers) as ``critical'', matching the specialised Transformer analyser.
For ResNets (2--8 residual blocks), the engine correctly identifies the near-critical phase ($\chi_1 \approx 1$) that skip connections induce.
Overall, the compositional engine achieves 83\% phase agreement (15/18) with specialised analysers.

\begin{table}[t]
\centering
\caption{Compositional mean-field engine: phase classification across 18 architectures. Phase agreement 83\% (15/18).}
\label{tab:graph-results}
\small
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Architecture} & \textbf{Ct.} & \textbf{Comp.} & \textbf{Std.} & \textbf{Agr.} \\
\midrule
MLP (3--10L)       & 9 & ord. & ord. & \checkmark \\
CNN (4L)           & 1 & ord. & ord. & \checkmark \\
ResNet (2--8B)     & 3 & crit. & ord.$^*$ & $\triangle$ \\
Transformer (2--6L) & 3 & crit. & crit. & \checkmark \\
Hybrid Conv+Trans. & 1 & crit. & crit. & \checkmark \\
Wide\&Deep         & 1 & ord. & ord. & \checkmark \\
\bottomrule
\end{tabular}

\smallskip
\noindent{\footnotesize $^*$Standard analyser uses MLP mean-field (predicts ordered); ResNets with skip connections are empirically critical. $\triangle$~Compositional engine is more accurate.}
\end{table}

\paragraph{Limitations of the compositional engine.}
For architectures with LayerNorm, the compositional engine's chi-product underestimates trainability because normalization resets variance, breaking the multiplicative chi model.
The specialised Transformer analyser (\S\ref{sec:transformer}) handles this correctly.
