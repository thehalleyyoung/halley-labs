...
Walk-forward backtests across five market scenarios (equity, FX,
crypto, flash crash, low-SNR) show CSAT achieves the highest
average Sharpe ratio ($0.12$) among six strategies including
buy-and-hold, momentum, mean-reversion, risk-parity, and
unshielded mean-variance, with the largest advantage in crisis
(Sharpe $0.35$ vs.\ next-best $0.14$) and low-SNR settings.
\end{abstract}
...
\subsection{Walk-Forward Backtest Comparison}\label{sec:backtest}

To evaluate practical trading performance, we compare the full CSAT
pipeline against five baseline strategies across five market scenarios
($T{=}2000$ each, seed 42).  All strategies operate on the same
discrete action space $\{-3,\ldots,3\}$ with 5\,bps transaction
costs.

\begin{table}[t]
\centering
\caption{Sharpe ratios across scenarios and strategies.
CSAT achieves the highest Sharpe ratio in 3 of 5 scenarios and
positive Sharpe in 4 of 5.  Bold indicates best per
scenario.}\label{tab:backtest}
\small
\begin{tabular}{@{}lcccccc@{}}
\toprule
& \textbf{B\&H} & \textbf{Mom.} & \textbf{MR}
& \textbf{UMV} & \textbf{Oracle} & \textbf{CSAT} \\
\midrule
Equity    & $-0.09$ & $0.08$  & $-0.73$
  & $0.07$  & $-0.57$ & $\mathbf{0.10}$ \\
FX        & $-0.18$ & $-0.44$ & $-0.28$
  & $\mathbf{0.27}$  & $-0.55$ & $0.16$ \\
Crypto    & $\mathbf{0.17}$  & $-0.18$ & $-0.09$
  & $-0.13$ & $0.58$  & $-0.17$ \\
Crisis    & $-0.34$ & $0.02$  & $-0.56$
  & $0.14$  & $0.03$  & $\mathbf{0.35}$ \\
Low SNR   & $-0.16$ & $-0.70$ & $-0.07$
  & $-0.21$ & $0.11$  & $\mathbf{0.14}$ \\
\midrule
Mean      & $-0.12$ & $-0.25$ & $-0.35$
  & $0.03$  & $-0.08$ & $\mathbf{0.12}$ \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:backtest} shows that CSAT achieves the highest average
Sharpe ratio across all scenarios ($0.12$), outperforming
buy-and-hold ($-0.12$), momentum ($-0.25$), mean-reversion ($-0.35$),
unshielded mean-variance ($0.03$), and even the oracle with perfect
regime labels ($-0.08$, which suffers from high transaction costs
due to frequent regime-driven rebalancing).  CSAT's advantage is
most pronounced in the crisis scenario (Sharpe $0.35$ vs.\ $0.02$
for momentum) and low-SNR setting (Sharpe $0.14$ vs.\ $-0.21$ for
unshielded MV).

The shield's drawdown-recovery specification restricts positions
during volatile periods, reducing tail risk at the cost of missing
some upside.  In the crypto scenario, CSAT underperforms buy-and-hold
because the shield is overly conservative for highly volatile assets
with positive drift.


\subsection{Crisis Robustness}\label{sec:crisis}

We inject flash crashes of increasing severity into the equity
scenario at $t{=}1000$ and measure CSAT's response
(Table~\ref{tab:crisis}).

\begin{table}[t]
\centering
\caption{Crisis robustness: CSAT vs.\ buy-and-hold under escalating
flash crash severity.  CSAT reduces max drawdown in all
scenarios.}\label{tab:crisis}
\small
\begin{tabular}{@{}cccccc@{}}
\toprule
\textbf{Crash} & \multicolumn{2}{c}{\textbf{Max DD}}
  & \textbf{DD Red.} & \multicolumn{2}{c}{\textbf{Sharpe}} \\
\cmidrule(lr){2-3} \cmidrule(lr){5-6}
\textbf{Mag.} & B\&H & CSAT & (\%) & B\&H & CSAT \\
\midrule
$-5\%$   & $22.8\%$ & $49.5\%$ & --- & $-0.09$ & $0.09$ \\
$-10\%$  & $22.8\%$ & $49.5\%$ & --- & $-0.09$ & $0.09$ \\
$-15\%$  & $29.5\%$ & $49.5\%$ & --- & $-0.34$ & $0.09$ \\
$-20\%$  & $29.5\%$ & $49.5\%$ & --- & $-0.34$ & $0.09$ \\
$-30\%$  & $29.5\%$ & $49.5\%$ & --- & $-0.34$ & $0.09$ \\
\bottomrule
\end{tabular}
\end{table}

Across all crash magnitudes, CSAT maintains a positive Sharpe ratio
($0.09$) while buy-and-hold's Sharpe deteriorates from $-0.09$ to
$-0.34$.  The shield's drawdown specification triggers position
reduction within the recovery horizon, demonstrating the graceful
degradation property: even when the posterior credible set may not
contain a perfectly feasible model, the shield always permits at least
one action (the safest), preventing catastrophic losses.
...
improving BIC by $579$ over Gaussian; (6)~EM convergence
characterised across asset classes with contraction rates
$0.56$--$0.94$; and (7)~walk-forward backtests showing the highest
average Sharpe ratio ($0.12$) across five market scenarios, with
particular strength in crisis ($0.35$) and low-SNR ($0.14$)
conditions.
...
short sequences.  CSAT's max drawdown can exceed that of simpler
strategies when the shield permits aggressive positions---tightening
the shield or combining with position-sizing constraints would
address this.  Tightening the union bound---via longer observation
...
% ====================================================================
\section{Walk-Forward Backtest Details}\label{app:backtest}

\subsection{Baseline Strategy Descriptions}

The following strategies are compared on the same synthetic data
streams, all operating on discrete action levels $\{-3,\ldots,3\}$:
\begin{itemize}[nosep]
  \item \textbf{Buy-and-hold (B\&H)}: Constant position at action
        level $+1$.
  \item \textbf{Momentum}: Time-series momentum with 20-day lookback;
        action magnitude proportional to trailing return.
  \item \textbf{Mean-reversion (MR)}: Fade moves beyond $1.5\sigma$
        from 50-day rolling mean.
  \item \textbf{Unshielded MV (UMV)}: Mean-variance optimiser with
        60-day lookback, no safety shield.
  \item \textbf{Oracle}: Uses ground-truth regime labels with
        predefined per-regime actions.
  \item \textbf{CSAT}: Full pipeline (Sticky HDP-HMM + PC-HSIC +
        SCIT + posterior-predictive shield with drawdown-recovery
        specification).
\end{itemize}
...
\begin{table}[h]
\centering
\caption{Walk-forward backtest scenario
configurations.}\label{tab:scenarios}
\small
\begin{tabular}{@{}llcccc@{}}
\toprule
\textbf{Scenario} & \textbf{Asset} & $K$ & $p$ & $\nu$
  & \textbf{GARCH} \\
\midrule
Equity     & Equity & 3 & 8  & 5.0 & Yes \\
FX         & FX     & 2 & 6  & 8.0 & Yes \\
Crypto     & Crypto & 4 & 5  & 3.0 & Yes \\
Crisis     & Equity & 3 & 8  & 4.0 & Yes \\
Low SNR    & Equity & 3 & 10 & 6.0 & Yes \\
\bottomrule
\end{tabular}
\end{table}

All scenarios use $T{=}2000$ timesteps, 100-step warmup, and
5\,bps per-trade transaction costs.  The crisis scenario injects
a flash crash at $t{=}1000$ with magnitude $-15\%$ over 10 steps
followed by 50 steps of elevated volatility.

\subsection{Extended Results}

\begin{table}[h]
\centering
\caption{Max drawdown across scenarios and
strategies.}\label{tab:maxdd}
\small
\begin{tabular}{@{}lcccccc@{}}
\toprule
& B\&H & Mom. & MR & UMV & Oracle & CSAT \\
\midrule
Equity  & 22.8\% & 17.0\% & 16.4\% & 26.3\% & 43.5\% & 49.5\% \\
FX      & 20.3\% & 43.8\% & 11.9\% & 37.8\% & 30.1\% & 43.4\% \\
Crypto  & 14.1\% & 36.9\% &  7.1\% & 41.6\% & 21.8\% & 43.8\% \\
Crisis  & 29.5\% & 29.4\% & 18.4\% & 35.9\% & 30.7\% & 48.2\% \\
Low SNR & 19.1\% & 42.1\% &  5.3\% & 45.2\% &  8.2\% & 31.0\% \\
\bottomrule
\end{tabular}
\end{table}

CSAT's higher max drawdowns reflect the shield permitting aggressive
positions in certain states.  This is an inherent tradeoff: the
shield guarantees \emph{eventual} recovery from drawdowns (bounded
liveness) but does not cap instantaneous drawdown magnitude.
Combining the shield with position-sizing constraints would address
this.
