...
def generate_data(
    ...
    # Random edge weights
    weights = adj.astype(np.float64) * rng.uniform(0.3, 0.9, size=(n, n))
...
def run_pipeline(
    ...
    if report.estimation_result is not None:
        er = report.estimation_result
        print(f"  Estimated ATE     : {er.estimate:.4f}  "
              f"(SE {er.std_error:.4f})")
...
def print_fragility_ranking(report: AuditReport, names: list[str]) -> None:
    ...
    for idx, fs in enumerate(ranking, 1):
        ...
        bar = "█" * int(fs.score * 40)
        print(f"  {idx:>2d}. {label:<30s}  {fs.score:.4f}  {bar}")
    print()
...
def identify_load_bearing_edges(
    ...
    for fs in report.fragility_ranking:
        if fs.score >= threshold:
            src, dst = fs.edge
            load_bearing.append((names[src], names[dst], fs.score))
