...
```bash
# Development tools (pytest, mypy, ruff, hypothesis)
pip install -e ".[dev]"
...
for fs in report.fragility_ranking[:3]:
    print(f"  Edge {fs.edge}: fragility={fs.score:.4f}")
```
...
weights = adj.astype(float) * 0.5   # uniform edge weight
data_array = generate_linear_gaussian(
    adj_matrix=adj, weights=weights,
    n_samples=2000, noise_scale=1.0, seed=42,
)
df = pd.DataFrame(data_array, columns=["C", "T", "M", "Y"])
```
...
#   solver_strategy: str      — "auto", "ilp", "lp_relaxation", "fpt", "cdcl"
...
from causalcert.dag.dsep import d_separated
from causalcert.dag.conversions import from_dot, from_json, to_dot
from causalcert.dag.edit import apply_edit
from causalcert.dag.moral import moralise
from causalcert.dag.mec import markov_equivalence_class
...
from causalcert.ci_testing.ensemble import CauchyEnsemble
...
from causalcert.solver.lp_relaxation import LPSolver   # ALG 5 — bounds
from causalcert.solver.fpt import FPTSolver             # ALG 7 — treewidth
from causalcert.solver.cdcl import CDCLSolver           # CDCL search
from causalcert.solver.search import GreedySolver       # greedy/brute-force
```
...
from causalcert.estimation.backdoor import BackdoorEstimator
from causalcert.estimation.adjustment import find_adjustment_set
```
...
from causalcert.reporting.latex_report import to_latex_report
```
...
| `solver_strategy` | `str` | `"auto"` | `"ilp"`, `"lp_relaxation"`, `"fpt"`, `"cdcl"`, `"auto"` |
